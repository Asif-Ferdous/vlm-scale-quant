...
%  (5,700 predictions); figures in . Compile:
...
images to a target selective accuracy is applied unchanged to each degraded
...
by 7B \emph{verbalized} confidence (grey dashed) is far weaker.}
\label{fig:rc}
\end{figure}
...
Because all arms are accurate and confident on most conditions, calibration-error
measures are low and similar across arms and cannot separate a discriminative
confidence signal from a near-constant one: a signal that is always $\approx\!0.9$
on a 90\%-accurate condition is well calibrated yet useless for ranking, with
AUROC near 0.5. This is exactly the regime of verbalized confidence here. Reliability
diagrams and ECE assess whether confidence \emph{magnitude} matches accuracy;
error-detection AUROC assesses whether confidence \emph{discriminates} correct
from incorrect answers. Only the latter distinguishes the arms, which is why we
adopt it as the primary metric and recommend it for future work on VLM confidence.
